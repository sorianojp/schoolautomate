...
Vector vRetResult = reportBIR.generateCashReceipt(dbOP, request);
if(vRetResult == null)
...
"<%=vRetResult.elementAt(i)%>","<%=vRetResult.elementAt(i + 1)%>","<%=vRetResult.elementAt(i + 2)%> ","<%=vRetResult.elementAt(i + 3)%>","<%=vRetResult.elementAt(i + 4)%>","<%=vRetResult.elementAt(i + 5)%>","<%=vRetResult.elementAt(i + 6)%>"<%if( (i + 7) < vRetResult.size()){%><br /><%}%>
<%}

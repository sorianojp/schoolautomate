...
<body leftmargin="20px" topmargin="80px" bottommargin="0" onLoad="PrintPg();">
<%
...
String[] astrConvertYearLevel = {"N/A", "1ST YEAR", "2ND YEAR", "3RD YEAR", "4TH YEAR", "5TH YEAR", "6TH YEAR"};
...
if(strSemester.length() == 0) 
	strSemester = WI.fillTextValue("offering_sem");
...
if(vStudInfo != null && vStudInfo.size() > 0) {
	...
	if(vInstallmentDtls != null && vInstallmentDtls.size() > 0)
		strAmountPaid = CommonUtil.formatFloat((String)vInstallmentDtls.elementAt(0),true);
	//System.out.println(strAmountPaid);
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">	
	<tr>
		<td height="30" valign="bottom" colspan="2"><%=(String)vStudInfo.elementAt(1)%></td>
		<td valign="bottom" width="15%" align="center"><%=strStudID%></td>
		...
		<%}else{%>
			<%=astrConvertYearLevel[Integer.parseInt(WI.getStrValue((String)vStudInfo.elementAt(4),"0"))]%>
		<%}%>
		</td>
	</tr>
	<%
	...
	%>
	<tr>
		<td height="30" valign="bottom" align=""><%if(!bolIsBasic){%><%=(String)vStudInfo.elementAt(19)%><%}%></td>
		<td valign="bottom" width="45%" align=""> &nbsp; &nbsp; <%if(!bolIsBasic){%><%=(String)vStudInfo.elementAt(16)%><%=WI.getStrValue((String)vStudInfo.elementAt(22), " - ", "", "")%><%}%></td>
		<td valign="bottom" width="15%" align="center"><%=astrConvertSem[Integer.parseInt(strSemester)]%></td>
		<td valign="bottom" width="15%" align="center"><%=strSYFrom+" - "+strSYTo%></td>
	</tr>
</table>
	...
	<tr>
		<td valign="top" height="280px">
			<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
				...
				<tr><td colspan="10" height="18">Total Amt. Pd. &nbsp; &nbsp; <%=WI.getStrValue(strAmountPaid)%></td></tr>
				<%
				   int iCount = 0;
				   if(!bolIsBasic) 
					   for(int i=1; i< vRetResult.size(); i += 11){
						...
						%>
						  <tr>
							<td height="18" width="25%"><%=strTemp%>. <%=WI.getStrValue(strErrMsg," &nbsp; ", "", "")%></td>
							<td height="18"><%=WI.getStrValue((String)vRetResult.elementAt(i+3), "N/A")%></td>			
						  </tr>
					  <%}%>
			</table>
		</td>
	...
	<tr>
		<td valign="top">
			<table width="100%" cellpadding="0" cellspacing="0">
				<tr>
					<td width="66%" align="center"><!--MARIA ROSA AMANDA N. REY--><%//=(String)request.getSession(false).getAttribute("first_name")%></td>

...
if(strErrMsg == null)
{
	...
	if(vScheduledPmt == null)
		...
		if(strPmtScheduleName.length() > 0) {
			iIndexOf = vScheduledPmt.indexOf(strPmtScheduleName);
			if(iIndexOf > -1)
...
String strSectionName = null;//set only if dbtc..
 if(vStudInfo != null && vStudInfo.size() > 0){
	 ...
	 if(strSchoolCode.startsWith("DBTC") || true) {
	 	if(bolIsBasic) {
...
%>
	  ...
	  <table width="100%" cellpadding="0" cellspacing="0" border="0">
        <tr>
          ...
          <td width="28%"><div align="right"><strong><%=CommonUtil.formatFloat((float)dBalance,true)%></strong>
		  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div></td>
        </tr>
        <tr>
          <td colspan="2">
...
strTemp = ConversionTable.convertMMDDYYYY((java.util.Date)vPayment.elementAt(i + 2));
...
if(	vPayment.elementAt(i + 1) == null) {%> <%=(String)vPayment.elementAt(i + 3)%> <%}else{
strTemp = (String)vPayment.elementAt(i + 3);
iIndexOf = strTemp.indexOf(" -");
if(iIndexOf > -1) {
	strTemp = strTemp.substring(0, iIndexOf);
	strTemp = strTemp.trim();
	if(strTemp.startsWith("Enrollment"))
		strTemp = "Downpayment";//System.out.println(strTemp);
}

%> <%=(String)vPayment.elementAt(i + 1)%> - <%=strTemp%><%}%></td>
...
if(vScheduledPmt != null && vScheduledPmt.size() > 0) {%>
        ...
        <tr>
		  ...
		  <%=strPmtScheduleName%> Examination:
		   <%}%>
		   <%=CommonUtil.formatFloat(dDueForThisPeriod,true)%></strong></font></td>
        </tr>
<%}//end of if condition%>
        <tr>
...
if(dDueForThisPeriod == 0d) { //added January, 13, 2014 .. to show other charges payable even though tuition is all paid.
	double dOthChargePayable = 0d;
	Vector vOtherChargeNotPaid = new Vector();
	
	java.sql.ResultSet rs    = null;
	String strSQLQuery = "select sum(fa_stud_payable.amount*no_of_units), fee_name from fa_stud_payable join fa_oth_sch_fee on (fa_oth_sch_fee.othsch_fee_index = fa_stud_payable.reference_index) where fa_stud_payable.is_valid = 1 and sy_from = "+request.getParameter("sy_from") + 
	...
	while(rs.next()) {
		...
		vOtherChargeNotPaid.addElement(rs.getString(2));//[1] name
		vOtherChargeNotPaid.addElement(new Double(rs.getDouble(1)));//[2] amount.
	}
	rs.close();
	...
	if(dOthChargePayable > 0d) {
		...
		strSQLQuery = "select sum(amount) from fa_stud_payment where is_valid = 1 and sy_from = "+request.getParameter("sy_from") + 
						" and semester = "+request.getParameter("semester")+" and user_index = "+(String)vStudInfo.elementAt(0)+
						" and amount > 0 and or_number is not null and (othsch_fee_index > 0 or payment_for = 8) and is_stud_temp = 0";
		**/
		strSQLQuery = "select sum(fa_stud_payment.amount), fee_name from fa_stud_payment "+
						"join fa_oth_sch_fee on (fa_oth_sch_fee.othsch_fee_index = fa_stud_payment.othsch_fee_index) "+
						" where fa_stud_payment.is_valid = 1 and sy_from = "+request.getParameter("sy_from") + 
						" and semester = "+request.getParameter("semester")+" and user_index = "+(String)vStudInfo.elementAt(0)+
						" and fa_stud_payment.amount > 0 and or_number is not null and is_stud_temp = 0 group by fee_name";
		
		rs = dbOP.executeQuery(strSQLQuery);
		int iIndexOf2 = 0;double dTempBal = 0d;
		
		while(rs.next()) {
			iIndexOf2 = vOtherChargeNotPaid.indexOf(rs.getString(2));
			if(iIndexOf2 == -1)
				continue;
			dOthChargePayable -= rs.getDouble(1);
			dTempBal = ((Double)vOtherChargeNotPaid.elementAt(iIndexOf2 + 1)).doubleValue() - rs.getDouble(1);
			if(dTempBal <= 0d) {
				///sometimes, excess payment is made for one kind of post charge, so, it makes the payable to -ve.. in case of sufficient payment is made, i have to 
				//remove the payable amt only.
				dOthChargePayable = dOthChargePayable + rs.getDouble(1) - ((Double)vOtherChargeNotPaid.elementAt(iIndexOf2 + 1)).doubleValue();

				vOtherChargeNotPaid.remove(iIndexOf2);vOtherChargeNotPaid.remove(iIndexOf2);
				continue;
			}
			vOtherChargeNotPaid.setElementAt(new Double(dTempBal), iIndexOf2 + 1);
		}
		rs.close();
	}
	...
	if(dOthChargePayable <=0d)
		...
		if(dDueForThisPeriod <= 0d)			
			dDueForThisPeriod += dOthChargePayable;
	}
}
...
if(dDueForThisPeriod > 0d) {
	...
	double dOthChargePayable = 0d;
	double dTotalOSPaid = 0d;
	
	//[0] oth_sch_fee_index, [1] fee_name, [2] amount
	Vector vOtherChargeNotPaid = new Vector();
	...
	if(dOSPayable >0d) {
		strSQLQuery = "select sum(amount), payment_for from fa_stud_payment where is_valid = 1 and sy_from = "+request.getParameter("sy_from") + 
						" and semester = "+request.getParameter("semester")+" and user_index = "+(String)vStudInfo.elementAt(0)+
						" and amount > 0 and or_number is not null and payment_for in (0, 10) and is_stud_temp = 0 group by payment_for";
		rs = dbOP.executeQuery(strSQLQuery);
		while(rs.next()) {
			if(rs.getInt(2) == 0)
				dTotalTuitionPaid = rs.getDouble(1);
			else	
				dTotalOSPaid = rs.getDouble(1);
		}
		rs.close();
		
		dOSPayable = dOSPayable - dTotalOSPaid;
		/**
...
%>
        <tr>
		  ...
		  <br>Total Charges: <%=CommonUtil.formatFloat(dOthChargePayable, true)%>
		  	<br><b><u>Details of Fee:</u></b>
		  	<%while(vOtherChargeNotPaid.size() > 0) {%>
				<br><%=vOtherChargeNotPaid.remove(0)%>: <%=CommonUtil.formatFloat(((Double)vOtherChargeNotPaid.remove(0)).doubleValue(), true)%>

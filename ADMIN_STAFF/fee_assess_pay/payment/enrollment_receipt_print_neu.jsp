...
<form name="form_" action="./enrollment_receipt_print_neu.jsp" method="post">
...
{//System.out.println(vStudInfo);
	...
	if(vMiscFeeInfo == null)
	 ...
	 //I have to check if student has paid add drop fee.
	 dAddDropFee = new enrollment.FAFeeMaintenance().addDropFeePayabaleBalance(dbOP, (String)vStudInfo.elementAt(0), astrSchYrInfo[0], astrSchYrInfo[2]);
	 if(dAddDropFee > 1d) {
	  strErrMsg = "Please pay add/drop fee before printing COM.";
	 }
}
if(strErrMsg == null) //collect fee details here.
{
	...
	if(fTutionFee > 0f)
	{
		...
		vDiscountAddlDetail = faStudLedg.getAdjustmentAndAllowance(new Vector(), dbOP, (String)vStudInfo.elementAt(0), (String)vORInfo.elementAt(23), 
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<tr>
	    <td width="69%" height="45" align="center" valign="bottom">&nbsp;</td>
	    <td width="31%" align="center" valign="bottom"><strong style="font-size:30px;">ENROLLED</strong></td>
	</tr>
	<tr>
		<td height="25" colspan="2" align="center" valign="bottom"><strong><%=astrConvertSem[Integer.parseInt(astrSchYrInfo[2])]%>, AY <%=astrSchYrInfo[0]+" - "+astrSchYrInfo[1]%></strong></td>
	</tr>
	
    <tr >
      <td height="20" align="right" colspan="3">Date and time printed: <%=WI.getTodaysDateTime()%>&nbsp;&nbsp;&nbsp;&nbsp;</td>
    </tr>
</table>
<% if(strErrMsg != null){%>
<table width="100%" border="0" cellpadding="0" cellspacing="0">
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" >
  <tr>
    <td height="18">Student Number </td>
  ...
  <tr>
    ...
    <td colspan="2"><%=WI.getStrValue(vStudInfo.elementAt(33),"N/A")%></td>
    </tr>
</table>
<%
if(vAssessedSubDetail != null && vAssessedSubDetail.size() > 0)
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="thinborder">
  <tr onDblClick="PrintAssessment()">
    <td align="center" width="10%" height="19" class="thinborder"><strong>SUBJECT CODE </strong></td>
    <td align="center" width="26%" height="19" class="thinborder"><strong>SUBJECT TITLE </strong></td>
	<td align="center" width="21%" class="thinborder"><strong>SCHEDULE</strong></td>
	<td align="center" width="15%" class="thinborder"><strong>SECTION</strong></td>
    <td align="center" width="15%" class="thinborder"><strong>ROOM</strong></td>
    <td align="center" width="5%" class="thinborder"><strong>LEC/LAB UNITS </strong></td>
    <td align="center" width="5%" class="thinborder"><strong>TOTAL UNITS</strong></td>
    <td align="center" width="6%" class="thinborder"><strong>RATE/UNIT</strong></td>
    <td align="center" width="7%" class="thinborder"><strong>TOTAL SUBJECT FEE </strong></td>
  </tr>
  <%
float fFirstInstalAmt = 0f;
float fTotalLoad = 0;
float fUnitsTaken = 0f;
float fTotalUnit = 0;
...
**/
		...
		if(vLabSched != null)
		{
		  for (int p = 0; p < vLabSched.size(); ++p)
		  {
			...
			strSection += "<br>" + (String) vLabSched.elementAt(p + 1) + "(lab)";
			p = p+ 2;
		  }
		}
%>
  <tr >
    ...
    <td class="thinborder"><%=WI.getStrValue(strSection,"N/A")%></td>
    <td class="thinborder"><%=WI.getStrValue(strRoom,"N/A")%>&nbsp;</td> 
...
strSchedule = null;
  ...
  <tr >
    <td colspan="9" height="18" class="thinborder"><div align="center">
        <%if(strErrMsg != null){%><%=strErrMsg%><%}else{%>TOTAL LOAD UNITS : <strong><%=fUnitsTaken%></strong><%}%>
      </div></td>
  </tr>
</table>
 ...
 float fAmtPaidDurEnrl = Float.parseFloat((String)vRetResult[1].elementAt(5));
	...
	if(bolIsFatima) {
		...
		if(iPrelimPercent > 0 || iMTPercent > 0 || iFinalPercent > 0) {
			...
			double dPrelimDue = (fTotalPayableAmt + dFatimaInstallmentFee - fAmtPaidDurEnrl) * (double)iPrelimPercent/100;
			int iIndexOf = vInstallmentDtls.indexOf("Prelim");
			if(iIndexOf > -1) {
				if(dPrelimDue == 0d) {
					vInstallmentDtls.remove(iIndexOf);vInstallmentDtls.remove(iIndexOf);vInstallmentDtls.remove(iIndexOf);
					...
					vInstallmentDtls.setElementAt(Double.toString(dPrelimDue + dAddDropFee), iIndexOf + 2);
				}
			}
			
			double dMTDue     = (fTotalPayableAmt + dFatimaInstallmentFee - fAmtPaidDurEnrl) * (double)iMTPercent/100;
			iIndexOf = vInstallmentDtls.indexOf("Midterm");
			if(iIndexOf > -1) {
				if(dMTDue == 0d) {
					vInstallmentDtls.remove(iIndexOf);vInstallmentDtls.remove(iIndexOf);vInstallmentDtls.remove(iIndexOf);
					...
					if(dPrelimDue == 0d)
						vInstallmentDtls.setElementAt(Double.toString(dMTDue + dAddDropFee), iIndexOf + 2);
					else
						vInstallmentDtls.setElementAt(Double.toString(dMTDue), iIndexOf + 2);
				}
			}
			double dFinal     = (fTotalPayableAmt + dFatimaInstallmentFee - fAmtPaidDurEnrl) * (double)iFinalPercent/100;
			iIndexOf = vInstallmentDtls.indexOf("Final");
			if(iIndexOf > -1) {
				if(dFinal == 0d) {
					vInstallmentDtls.remove(iIndexOf);vInstallmentDtls.remove(iIndexOf);vInstallmentDtls.remove(iIndexOf);
					...
					vInstallmentDtls.setElementAt(Double.toString(dFinal), iIndexOf + 2);
			}
		}	
    ...
    <td width="45%" height="14" valign="top">&nbsp;
     ...
     <%}if(bolShowMiscDtls) {%>
		...
		for(int i = 0; i< vMiscFeeInfo.size(); i +=3){
			if( WI.getStrValue((String)vMiscFeeInfo.elementAt(i + 2)).compareTo("1") ==0 || !bolShowMiscDtls)
				continue;
		%>
 		<tr>
		...
		<%}%>
        <tr>
          <td height="14"><strong>TOTAL MISCELLANEOUS FEES</strong></td>
		...
		for(int i = 0; i< vMiscFeeInfo.size(); i +=3){
			if(strSchoolCode.startsWith("CSA"))
				continue;
			if( WI.getStrValue((String)vMiscFeeInfo.elementAt(i + 2)).compareTo("0") ==0 || !bolShowOthChargeDtls)
				continue;
		%>
        <tr>
          <td height="14">&nbsp;&nbsp;<font size="1"><%=(String)vMiscFeeInfo.elementAt(i)%></font></td>
...
<%}%>
        <tr>
          ...
          <td height="14"><div align="right"><font size="1"><strong>Php <%=CommonUtil.formatFloat(fTotalPayableAmt + dDPFineCGH + dFatimaInstallmentFee - fAmtPaidDurEnrl + dAddDropFee ,true)%></strong></font></div></td>
        </tr>
        <tr>
          <td height="14">&nbsp;</td>
          <td height="14"><div align="right"></div></td>
...
<%}%>
        ...
        <%} if(vRetResult[1].elementAt(1) != null && !WI.getStrValue(vRetResult[1].elementAt(1)).equals("Internal") && false){%>
        <tr>
       ...
       <tr>
	   		...
	   		<td colspan="2" valign="top">
				<table width="100%" cellpadding="0" cellspacing="0" class="thinborder">
					...
					<tr>
						<td valign="top" width="31%" class="thinborder">
						<%=WI.getStrValue((String)vInstallmentDtls.elementAt(i)).toUpperCase()%><br>
						<strong><%=CommonUtil.formatFloat((String)vInstallmentDtls.elementAt(i+2),true)%></strong>
						<br>&nbsp;<br>&nbsp;
						</td>
						<td valign="top" width="33%" class="thinborder">On or before<br>
						    <strong><%=WI.getStrValue((String)vInstallmentDtls.elementAt(i+1)).toUpperCase()%></strong></td>
						<td valign="top" width="36%" class="thinborder">&nbsp;</td>
					</tr>
					<%}%>
				...
				<tr><td><strong>Discount/Scholarship/Grants Information:</strong></td></tr>
				</table>
				<table width="100%" cellpadding="0" cellspacing="0" class="thinborder">
					<%for(int i = 0; i < vDiscountAddlDetail.size(); i+=7){
						strTemp = WI.getStrValue(vDiscountAddlDetail.elementAt(i));
						strErrMsg = null;
						if(strTemp.length() > 0 && strTemp.indexOf(":::") > -1){
							strErrMsg = strTemp.substring(strTemp.indexOf(":::")+3);
							strTemp = strTemp.substring(0,strTemp.indexOf(":::"));
						}
					%>
					<tr>
						<td width="30%" class="thinborder"><%=strTemp%></td>
						<td class="thinborder"><%=WI.getStrValue(strErrMsg,"&nbsp;")%></td>
						<td width="30%" class="thinborder"><%=CommonUtil.formatFloat((String)vDiscountAddlDetail.elementAt(i+1),true)%></td>
					</tr>
					<%}%>
				</table>
				<%}%>

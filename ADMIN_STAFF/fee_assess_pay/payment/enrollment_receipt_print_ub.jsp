...
int iIndexOf = 0;
Vector vNonCreditSubject = new enrollment.GradeSystem().getMathEngEnrichmentInfo(dbOP, request);
if(vNonCreditSubject == null)
	vNonCreditSubject = new Vector();
...
%>
    <tr >
      <td height="20" colspan="2" style="font-size:12px;">
	  <div style="float:left; position:absolute; top:10px; left:100px;"><img src="../../../images/logo/<%=strSchoolCode%>.gif" border="0" height="80"></div>
	  <div align="center"><strong><%=strTemp%></strong><br>
        <%=astrConvertSem[Integer.parseInt(astrSchYrInfo[2])]%> AY <%=astrSchYrInfo[0]+" - "+astrSchYrInfo[1]%>
		</div></td>
    </tr>
<%}%>	
...
if(strSchoolCode.startsWith("UI") && fOperation.vAssessedHrDetail != null &&
    ...
    <%}
if(false && !strSchoolCode.startsWith("AUF") && !strSchoolCode.startsWith("UDMC") && !strSchoolCode.startsWith("CSA") && !strSchoolCode.startsWith("FATIMA")) {%>
...
String strLecLabStat = null;//0 = both,1 = lab, 2 = lec.
	...
	for(int i = 0; i< vAssessedSubDetail.size() ; ++i)
	{
		...
		//check if it is nonCredit subject.
		if(Double.parseDouble((String)vAssessedSubDetail.elementAt(i+9)) == 0d) {
			//check if it is non-credit subject. 
			...
			iIndexOf = vNonCreditSubject.indexOf(strTemp);
			if(iIndexOf > 0) {
				double dTempNonCreditUnit = Double.parseDouble((String)vNonCreditSubject.elementAt(iIndexOf+1)) + Double.parseDouble((String)vNonCreditSubject.elementAt(iIndexOf+2));
				if(dTempNonCreditUnit > 0d) {
					vAssessedSubDetail.setElementAt("("+Double.toString(dTempNonCreditUnit)+")",i + 9);
					dNonCreditUnit += dTempNonCreditUnit;
				}
...
strSchedule = null;
  ...
  <tr >
    <td colspan="7" height="18" class="thinborder"><div align="center">
        ...
        <!--<%=fTotalLoad%>/-->
        <%=fUnitsTaken%>
		<%if(dNonCreditUnit > 0d) {
			strTemp = CommonUtil.formatFloat((double)fUnitsTaken + dNonCreditUnit, false);
    ...
    <td width="45%" height="14" valign="top">&nbsp;
     ...
     <%}if(bolShowMiscDtls) {%>
       ...
       <%if(strSchoolCode.startsWith("AUF") || strSchoolCode.startsWith("UDMC")){%>
        ...
        <tr>
          <td height="14"><font size="1">TOTAL OTHER CHARGE</font></td>
...
<%}//do not show for WNU%>		
 <%if(!strSchoolCode.startsWith("UL") && !strInfo5.equals("jonelta") && !strInfo5.equals("UB")){
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    ...
    <td valign="top"><em>Dean / Faculty/Secretary</em></td>
    <td valign="top">&nbsp;</td>
    <td valign="top" class="thinborderTOP" align="center" style="font-size:9px;"><em>College Dean/Dept. Head</em></td>
  </tr>
</table>

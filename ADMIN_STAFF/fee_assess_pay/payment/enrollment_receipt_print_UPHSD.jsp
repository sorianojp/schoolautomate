...
<body onLoad="window.print();" leftmargin="0" topmargin="0" bottommargin="0" rightmargin="0">
<%
...
boolean bolApplyFormat2 = false;//apply this format for new student only.. 
if(vStudInfo != null && vStudInfo.size() > 14 && vStudInfo.elementAt(15) != null) {
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" >
  ...
  <tr>
	...
	<!--------------STUDENT NAME------------------->
  	<td width="29%" align="left">&nbsp;<%=(String)vStudInfo.elementAt(1)%></td>
    <td width="5%">&nbsp;</td>	
  ...
  <tr>
  	<td height="18" width="11%">&nbsp;</td>
	<!--------------STUDENT NAME------------------->
  	<td width="29%" align="left">&nbsp;<%=(String)vORInfo.elementAt(25)%></td>
    <td width="5%">&nbsp;</td>	
    <td width="5%">&nbsp;</td>	
    <td width="10%">&nbsp;</td>	
    <td width="15%">&nbsp;</td>
  ...
  <tr>
	...
	<td width="10%">&nbsp;</td>
	<td width="15%"><strong><%=(String)vRetResult[1].elementAt(7)%></strong></td>
  </tr>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="thinborder">
  <tr >
  	<td width="7%" height="40" class="thinborder"><strong>&nbsp;<!--SUBJECT CODE--> </strong></td>
    <td width="10%" class="thinborder"><strong>&nbsp;<!--SUBJECT CODE--> </strong></td>
    <td width="31%" class="thinborder"><strong>&nbsp;<!--SUBJECT TITLE--> </strong></td>
	<%if(!strSchoolCode.startsWith("AUF") && !strSchoolCode.startsWith("UDMC")){%>
    <td width="5%" class="thinborder"><strong>&nbsp;<!--LEC/LAB UNITS --></strong></td>
    <%}%>
	
    <td width="22%" class="thinborder"><strong>&nbsp;<!--SCHEDULE--></strong></td>
    <td width="13%" class="thinborder"><strong>&nbsp;<!--ROOM #--></strong></td>
    
<%if(!bolApplyFormat2){%>
	<td width="12%" class="thinborder"><strong>&nbsp;<!--TOTAL SUBJECT FEE--></strong></td>
<%}%>
  </tr>
  <%//System.out.println(vAssessedSubDetail);
...
**/
		...
		for(int b=0; b<vSubSecDtls.size(); ++b)
		{
			...
			if(vSubSecDtls.elementAt(b) != null)
				strSubSecIndex = (String)vSubSecDtls.elementAt(b);
...
%>
  <tr >
  	<td height="19" class="thinborder"><%=strSubSecIndex%></td>
	...
	<%if(!strSchoolCode.startsWith("AUF") && !strSchoolCode.startsWith("UDMC")) {%>
    <td class="thinborder" align="center"><%=(String)vAssessedSubDetail.elementAt(i+9)%> <!--<%=(String)vAssessedSubDetail.elementAt(i+3)%>/<%=(String)vAssessedSubDetail.elementAt(i+4)%>--></td>
    <%}%>
...
strSchedule = null;
  ...
  <tr >
    <td height="20" colspan="4" style="border-top: solid 1px #000000; border-bottom: solid 1px #000000;"><div align="right">
        <%if(strErrMsg != null){%>
        <%=strErrMsg%>
        <%}else{%>
        <strong>
        <!--<%=fTotalLoad%>/-->
        <%=fUnitsTaken%> Units</strong>
        <%}%>		
      </div></td>
	  <td colspan="2" align="right" style="border-top: solid 1px #000000; border-bottom: solid 1px #000000;">&nbsp;
	  <%if(!bolApplyFormat2) {%>
	  	Subject Amount &nbsp;&nbsp;&nbsp; : &nbsp;&nbsp;&nbsp; 
	  <%}%>
	  </td>
	  <%if(!bolApplyFormat2) {%>
	  <td colspan="" align="right" style="border-top: solid 1px #000000; border-bottom: solid 1px #000000;">
	  	<strong><%=CommonUtil.formatFloat(fTutionFee,true)%></strong>
	  </td>
	  <%}%>	
  </tr>  
</table>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" >
  <tr>
    <td width="32%" valign="top">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" >
			<%for(int i = 0; i < vMiscFeeInfo.size(); i += 9){
				if( ((String)vMiscFeeInfo.elementAt(i + 2)).compareTo("1") ==0)
					continue;
			%>
			  <tr>
				<td width="75%" height="19"><%=(String)vMiscFeeInfo.elementAt(i)%></td>
				<td align="right">
				<%if(!bolApplyFormat2) {%>
					<%=CommonUtil.formatFloat((String)vMiscFeeInfo.elementAt(i+1),true)%>
				<%}else{%>&nbsp;<%}%>
				</td>
			  </tr>
			<%}%>
    ...
    <td width="32%" valign="top">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" >
			<%for(int i = 3; i < vMiscFeeInfo.size(); i += 9){
				if( ((String)vMiscFeeInfo.elementAt(i + 2)).compareTo("1") ==0)
					continue;
			%>
			  <tr>
    ...
    <td width="32%" valign="top">
		<table width="100%" border="0" cellpadding="0" cellspacing="0" >
			<%for(int i = 6; i < vMiscFeeInfo.size(); i += 9){
				if( ((String)vMiscFeeInfo.elementAt(i + 2)).compareTo("1") ==0)
					continue;
			%>
			  <tr>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" >
  <tr >
    <td height="20" colspan="4" style="border-top: solid 1px #000000; border-bottom: solid 1px #000000;" align="right">
<%if(!bolApplyFormat2) {%>
		<strong><%=CommonUtil.formatFloat(fCompLabFee + fMiscFee - fMiscOtherFee,true)%></strong>
<%}else{%>&nbsp;<%}%>
	</td>
  </tr>  
</table>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" >
  <tr>
    ...
    <td width="34%" valign="top">&nbsp;
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
          <td width="77%" height="19">TOTAL ASSESSMENT :</td>
          <td width="23%" align="right"><strong>
<%if(!bolApplyFormat2) {%>
			  <%=CommonUtil.formatFloat(fTutionFee+fCompLabFee+fMiscFee + dFatimaInstallmentFee,true)%>
<%}else{%>
			  <%=CommonUtil.formatFloat(fTutionFee+fCompLabFee+fMiscFee + dFatimaInstallmentFee - fMiscOtherFee,true)%>
<%}%>
		  </strong></td>
        </tr>
<%if(fMiscOtherFee > 0f){%>
        <tr>
          <td height="19">OTHER CHARGES : </td>
          <td align="right"><%=CommonUtil.formatFloat(fMiscOtherFee,true)%></td>
        </tr>
<%}%>
        <tr>
          <td height="19">OLD ACCOUNT : </td>
          <td align="right"><%=CommonUtil.formatFloat(fOutstanding,true)%></td>
        </tr>
        <tr>
          <td height="19">SCHOLARSHIP/DISCOUNT :</td>
          <td align="right"><%=CommonUtil.formatFloat(dTotalDiscount + fEnrollmentDiscount,true)%></td>
        </tr>
        <tr>
          <td height="19">DOWNPAYMENT :</td>
          <td align="right"><%=CommonUtil.formatFloat((String)vRetResult[1].elementAt(5),true)%></td>
        </tr>
        <tr>
          <td height="19">BALANCE : </td>
          <td align="right"><%=CommonUtil.formatFloat(fTutionFee+fCompLabFee+fMiscFee + fOutstanding  - fEnrollmentDiscount - Float.parseFloat((String)vRetResult[1].elementAt(5)) - dTotalDiscount,true)%></td>
        </tr>
...
<table width="100%" cellpadding="0" cellspacing="0" class="thinborderALL">
	<tr>
	  <td align="center" height="30px" valign="bottom"><font size="4"><strong>YOU'RE NOW ENROLLED, WELCOME !</strong></font></td>
	</tr>
</table>
<%

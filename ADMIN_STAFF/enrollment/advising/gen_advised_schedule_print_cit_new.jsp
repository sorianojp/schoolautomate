...
//new condition added -- May, 06, 2014 --> if freshman is underload, only ETO will prit the form.
if(!bolFatalErr && !bolIsConfirmed && vStudInfo != null && vStudInfo.elementAt(2) != null && !astrSchYrInfo[2].equals("0") && 
	strMaxAllowedLoad != null && !strMaxAllowedLoad.equals("N/A") && strMaxAllowedLoad.length() > 0 && vAdvisedList != null && vAdvisedList.size() > 0) { 
	...
	boolean bolIsFreshMan = false;
	if(strTemp.startsWith("N")) {
		strTemp = "select degree_type from course_offered where course_index = "+vStudInfo.elementAt(2);
		//System.out.println(strTemp);
		strTemp = dbOP.getResultOfAQuery(strTemp, 0);
		if(strTemp != null && strTemp.equals("0"))
			bolIsFreshMan = true;
	}
	double dMaxLoad = 0d; double dUnitEnrolled = 0d;
	try {
		dMaxLoad = Double.parseDouble(strMaxAllowedLoad);
		dUnitEnrolled = Double.parseDouble((String)vAdvisedList.elementAt(0));
	}
	catch(Exception e) {}
	//System.out.println("Printing: "+dUnitEnrolled);
	if(bolIsFreshMan && dMaxLoad > 0d && dMaxLoad > dUnitEnrolled) {
		//if not ETO, stop/
		boolean bolIsETO = new enrollment.SetParameter().bolIsETO(dbOP, (String)request.getSession(false).getAttribute("userIndex"));
		//System.out.println(bolIsETO);
		if(!bolIsETO) {
			bolFatalErr = true;
			strErrMsg = "<font size='3'>Printing of Underload Freshman is not allowed in colleges. Please proceed to ETO for printing.</font>";
		}
...
if(strCollegeCode != null) {
	if(strCollegeCode.toLowerCase().equals("cea") || strCollegeCode.toLowerCase().equals("cas") || strCollegeCode.toLowerCase().equals("con"))
		bolShowHeader = true;
}
%>
...
<body onLoad="<%if(!bolFatalErr){%>window.print();<%}%>">
<%if(bolFatalErr){%>
...
<%if(bolShowHeader){%>
<table cellpadding="0" cellspacing="0" border="0" width="90%" height="45">
	<tr valign="top" style="font-weight:bold">
		<td width="20%">REGISTRATION CARD</td>
		<td align="center"><img src="./cit_header.gif" width="293" height="32"></td>
		<td width="20%" align="right"><%=WI.getStrValue(strCollegeName, "(",")","").toUpperCase()%></td>
	</tr>
</table>
<%}%>
  <table width="90%" border="0" cellpadding="0" cellspacing="0">
...
<table cellpadding="0" cellspacing="0" border="0" width="90%" height="45">
	<tr valign="top" style="font-weight:bold">
		<td width="20%">REGISTRATION CARD</td>
		<td align="center"><img src="./cit_header.gif" width="293" height="32"></td>
		<td width="25%" align="right"><%=WI.getStrValue(strCollegeName, "(",")","").toUpperCase()%></td>
	</tr>
</table>
<%}%>
<table width="90%" border="0" cellpadding="0" cellspacing="0">

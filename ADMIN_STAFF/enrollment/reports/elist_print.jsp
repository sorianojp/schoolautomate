...
//	if(strSchCode.startsWith("DBTC"))
	...
	if(strSchCode.startsWith("PHILCST")){%>
		<jsp:forward page="./elist_print_PHILCST.jsp"/>
	<%}
	if(strSchCode.startsWith("PWC")){%>
		<jsp:forward page="./elist_print_PWC.jsp"/>
	<%}
	if(strSchCode.startsWith("SPC")){%>
		<jsp:forward page="./elist_print_SPC.jsp"/>
	<%}
	...
	if(strSchCode.startsWith("SWU")){%>
		<jsp:forward page="./elist_print_SWU.jsp"/>
	<%
	return;}
	...
	<%
	return;}	
	if(strSchCode.startsWith("SWU") && false){%>
		<jsp:forward page="./elist_print_new_SWU.jsp"/>
	<%
	return;}	
	if(strSchCode.startsWith("UB")){%>
		<jsp:forward page="./elist_print_UB.jsp"/>
	<%}
...
<%
	...
	if(strSchCode.startsWith("SWU")){
		...
		astrConvertSem[4] = "FOURTH SEMESTER";
		astrConvertSem[5] = "FIFTH SEMESTER";
...
if(strSchCode.startsWith("CGH") || strSchCode.startsWith("UDMC") || strSchCode.startsWith("SWU")) {
...
boolean bolShowCollegeInfo = false;
if(strSchCode.startsWith("SWU"))
	bolShowCollegeInfo = true;

Vector vRetResult = null;
//8 subjects in one row -- change it for different no of subjects per row
if(strSchCode.startsWith("SWU"))
	vRetResult = enrlReport.getEnrollmentListCSA(dbOP,request);
else
	vRetResult = enrlReport.getEnrollmentList(dbOP,request,iNoOfSubPerRow,bolSeparateName, bolShowCollegeInfo);
...
if(strSchCode.startsWith("SWU"))
	strPgCountDisp = "Page "+Integer.toString(iPageCount++) +" of "+Integer.toString(iTotalNoOfPage);
else
...
<table width=100% border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		<td style="font-weight:bold"><%=SchoolInformation.getSchoolName(dbOP,true,false)%></td>
		<%
		strTemp = "select TEL_NO from sys_info";
		strTemp = dbOP.getResultOfAQuery(strTemp, 0);		
		%>
	    <td style="font-weight:bold">Tel. No. <b><%=WI.getStrValue(strTemp, "871 0639")%></b></td>
	</tr>
	<tr>
		<td height="20">Address:</td>
		<td colspan="2" style="font-weight:bold"><%=WI.getStrValue(SchoolInformation.getAddressLine1(dbOP,false,false),"&nbsp;")%></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<%
		strTemp = "select SCH_IDENTIFIER from sys_info";
		strTemp = dbOP.getResultOfAQuery(strTemp, 0);		
		%>
		<td width="56%" style="font-weight:bold"><%=WI.getStrValue(strTemp, "13136")%></td>
	    <td width="24%">&nbsp;</td>
	</tr>
	<tr>
...
%>
  <tr>
    ...
    <td class="thinborder"><%=WI.getStrValue((String)vEnrlInfo.remove(3), "&nbsp;")%></td>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="">
  <tr>
    <td width="5%" height="24" class="thinborderTOPBOTTOM" align="center">No.</td>
    <td width="15%" class="thinborderTOPBOTTOM">Student No</td>
    <td colspan="3" class="thinborderTOPBOTTOM">Name</td>
    <td width="5%" class="thinborderTOPBOTTOM">Sex</td>
	<td width="5%" class="thinborderTOPBOTTOM">Year</td>
	<td width="5%" class="thinborderTOPBOTTOM">Course</td>
...
%>
  <tr>
    <td height="21" class=""><%=iStudCount++%>.</td>
    <td class=""><%=(String)vEnrlInfo.remove(0)%></td>
	<%
	strTemp =  (String)vEnrlInfo.remove(5);
	strTemp = WebInterface.formatName((String)vEnrlInfo.remove(3), (String)vEnrlInfo.remove(3),strTemp, 4);
	%>
    <td colspan="3" class=""><%=strTemp%></td>    
    <td class=""><%=(String)vEnrlInfo.remove(1)%></td>
	<td class=""><%=(String)vEnrlInfo.remove(4)%></td>
	<td class=""><%=(String)vEnrlInfo.remove(2)%></td>
    <td class="">
	<%//print all subjects enrolled.
	vEnrlInfo.removeElementAt(0);vEnrlInfo.removeElementAt(0);vEnrlInfo.removeElementAt(0);//now i have combination of sub code and units in list.
	fUnitEnrolled = 0f;
	strTemp = null;
	for(int k=0; k < vEnrlInfo.size() ; k+= 2){//only sub and units enrolled.
		if(strTemp == null)
	...
	<%=strTemp%>	</td>
    <td align="center" class=""><%=fUnitEnrolled%></td>
  </tr>
  <%
 }//end of print per page.%>
</table>
...
if(strSchCode.startsWith("CGH") || strSchCode.startsWith("UDMC") || strSchCode.startsWith("SWU")){%>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
<%if(strSchCode.startsWith("CGH") || strSchCode.startsWith("SWU")){%>
  <tr>
	...
	if(strSchCode.startsWith("SWU"))
		strTemp = "left";
	%>
	<td colspan="2" align="<%=strTemp%>"><%=strPgCountDisp%></td>
  </tr>
...
&nbsp;
	...
	<td width="461" align="right"><%if(!strSchCode.startsWith("SWU")){%><%=strPgCountDisp%><%}%></td>
  </tr>

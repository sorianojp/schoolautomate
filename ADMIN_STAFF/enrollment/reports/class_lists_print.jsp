...
<%
strTemp = WI.fillTextValue("subject_name");
...
%>
  <tr > 
    <td width="42%" height="10" >Subject code : <%=strTemp%></td>
...
<%if(bolIsHTC){
String[] astrTerm = {"ALL TERM","FIRST TERM","SECOND TERM"};
String strTerm = "ALL TERM";
enrollment.GradeSystem GS = new enrollment.GradeSystem();
strTemp = "select sub_index, TERM_ESS from e_sub_section where sub_sec_index = "+request.getParameter("section");
rs = dbOP.executeQuery(strTemp);
strTemp = null;
if(rs.next()){
	strTemp = rs.getString(1);
	strTerm = astrTerm[rs.getInt(2)];
}rs.close();

%>
  <tr >
      <td height="25" >UNITS : <%=WI.getStrValue(GS.getLoadingForSubject(dbOP, strTemp))%></td>
      <td height="25" >TERM : <%=strTerm%></td>
  </tr>
<%}%>
  <tr > 

...
boolean bolShowAdmSlip = false;
if(WI.fillTextValue("show_adm_no").length() > 0) 
	bolShowAdmSlip = true;
...
//I have to get the admission slip if already printed.. 
int iIndexOf = 0;
Vector vAdmSlip = new Vector();
if(bolShowAdmSlip) {
	String strSQLQuery = "select id_number, ADM_SLIP_NUMBER from ADM_SLIP_PRN_COUNT "+
						"join USER_TABLE on (user_Table.USER_INDEX = adm_slip_prn_count.USER_INDEX) "+
						"where PMT_SCH_INDEX = "+WI.fillTextValue("pmt_schedule")+" and SY_FROM = "+WI.fillTextValue("sy_from")+
						" and SEMESTER = "+WI.fillTextValue("semester")+" and ADM_SLIP_NUMBER <> '0'";
	java.sql.ResultSet rs = dbOP.executeQuery(strSQLQuery);
	while(rs.next()) {
		vAdmSlip.addElement(rs.getString(1));
		vAdmSlip.addElement(rs.getString(2));
	}
	rs.close();
}

boolean bolGetUBGraduatingOnly = false;
if(WI.fillTextValue("show_graduating").length() > 0) 
	bolGetUBGraduatingOnly = true;
...
while(vSubSecRef.size() > 0) {
	...
	vClassList = reportEnrl.getClassListDetailCIT(dbOP, strSubSecIndex, WI.fillTextValue("sy_from"), WI.fillTextValue("semester"), bolGetUBGraduatingOnly);
		  ...
		  <tr>
			...
			<table width="100%" cellpadding="0" cellspacing="0">
				<tr>
				    ...
				    <%if(bolShowAdmSlip) {%>
						<td width="8%" class="thinborderTOPBOTTOM">Admission Slip #</td>
					<%}%>
				</tr>
				<%
				...
				for(iRowsPrinted = 0; i < vClassList.size(); i += 7, ++iRowsPrinted) {
					...
					<tr>
						...
						<td><%=vClassList.elementAt(i + 3)%><%=WI.getStrValue((String)vClassList.elementAt(i + 4), " - ", "","")%></td>
						<td><%=WI.getStrValue((String)vClassList.elementAt(i + 5),"&nbsp;")%></td>
				    	<%if(bolShowAdmSlip) {
							iIndexOf = vAdmSlip.indexOf(vClassList.elementAt(i));
							if(iIndexOf == -1) 
								strTemp = null;
							else {
								//vAdmSlip.remove(iIndexOf);	
								//strTemp = (String)vAdmSlip.remove(iIndexOf);
								strTemp = (String)vAdmSlip.elementAt(iIndexOf + 1);
							}
						%>

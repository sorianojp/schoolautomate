...
<html>
  ...
  <%
    ...
    DBOperation dbOP = new DBOperation((String)request.getSession(false).getAttribute("userId"),
        "PURCHASING-PURCHASE ORDER","entrance_admission_slip_print.jsp");
    ...
    request.setAttribute("sy_from",request.getParameter("sy_from"));
    request.setAttribute("sy_to",request.getParameter("sy_to"));
    request.setAttribute("semester",request.getParameter("semester"));
    request.setAttribute("course_index",request.getParameter("course_index"));
    request.setAttribute("major_index",request.getParameter("major_index"));
    request.setAttribute("year_level",request.getParameter("year_level"));
    request.setAttribute("prep_prop_stat",request.getParameter("prep_prop_stat"));

    if (strTempID.length() > 0){
      vRetResult = offlineAdd.updatePlacementHSAve(dbOP,request,strTempID);
      if (vRetResult == null)
...
<%if(bolIsWNU){%>
      <tr>
		...
		<strong>INSTITUTE OF LANGUAGES</strong><br>
		<strong>EPT Individual Result</strong><br>
		<strong>School Year: <%=astrSemester[Integer.parseInt(WI.getStrValue((String)vRetResult.elementAt(12),"1"))]%> &nbsp;&nbsp;<%=(String)vRetResult.elementAt(0)%> - <%=strSYTo%></strong></td>
      </tr>
<%}else{%>
      <tr> 
        <td height="2">&nbsp;</td>
      ...
      <tr> 
        ...
        <td colspan="2">
		<font size="3">
		<%=SchoolInformation.getSchoolName(dbOP,false,false)%></font></td>
      </tr>
      <tr> 
        <td height="2">&nbsp;</td>
...
<%}%>      <tr> 
      ...
      <tr> 
        <td height="25">&nbsp;</td>
        <td height="25"><%if(bolIsWNU){%>Total Score<%}else{%>Placement Exams<%}%> : <strong><%=WI.getStrValue((String)vRetResult.elementAt(4))%></strong></td>
      ...
      <tr> 
        <td height="25">&nbsp;</td>
        <td height="25"><%if(bolIsWNU){%><b><u>Remarks : <%=WI.fillTextValue("remark_")%></u></b><%}%></td> 
<%
strTemp = null;
if(bolIsWNU)
...
%>
        <td height="25"><div align="center">__<u>DR <%=strTemp%></u>__</div></td>
      </tr>
      <tr> 
        <td height="25">&nbsp;</td>

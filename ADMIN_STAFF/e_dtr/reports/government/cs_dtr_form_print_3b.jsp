...
<%
	...
	try
	{
		dbOP = new DBOperation((String)request.getSession(false).getAttribute("userId"),
								"Admin/staff-eDaily Time Record-STATISTICS & REPORTS","cs_dtr_form_print_3b.jsp");
	}
	catch(Exception exp)
	{
		exp.printStackTrace();
...
String strAmTimeIn2 = null;
String strAmTimeOut2 = null;
String strPmTimeIn2 = null;
String strPmTimeOut2 = null;
...
if (WI.fillTextValue("my_home").equals("1"))
		strEmpID = WI.getStrValue((String)request.getSession(false).getAttribute("userId"),"");
else
...
//	if(WI.fillTextValue("searchEmployee").length() > 0 && strEmpIndex.length() > 0){
...
<%}%>
  ...
  <tr>
    <td width="47%">
		...
		//System.out.println("\n336 strDateFr " + strDateFr);
		//System.out.println("336 strDateTo " + strDateTo);
	...
	//System.out.println("\n4120 strDateFr " + strDateFr);
	//System.out.println("4120 strDateTo " + strDateTo);
	//System.out.println("vEmpDetails " + vEmpDetails);
	//System.out.println("errMsg " + rptExtn.getErrMsg());
...
%>
		...
		<table  bgcolor="#FFFFFF" width="100%" border="0" cellspacing="0" cellpadding="0" id="main_label">
		<tr>
		<td>
...
//	Vector vLateUnderTime = RE.getMonthlyLateUTimeDetail(dbOP, request);
	...
	for(int iIndex = 0; iIndex< iSize ;iIndex +=8){
		...
		if(bolSkipAm){
			...
			strUTEntry = "0";		
				...
				if(strDateOut != null && !strDateIn.equals(strDateOut)){
					...
					strAmTimeIn2 = "<br>(" + WI.formatDate(calendar.getTime(), 1) + ")";
					
					calendar.setTimeInMillis(lTempOutAm);
					strAmTimeOut2 = "<br>(" + WI.formatDate(calendar.getTime(), 1) + ")";
				}					
		}//<sul
		...
		//>sul..additional code for next day log-out..sul02072013
...
//		strPmTimeOut = ConversionTable.replaceString(strPmTimeOut," ","");
		...
		if(strDateOut != null && !strDateIn.equals(strDateOut)){
				...
				strPmTimeIn2 = "<br>(" + WI.formatDate(calendar.getTime(), 1) + ")";
				
				calendar.setTimeInMillis(lTempOutPm);
				strPmTimeOut2 = "<br>(" + WI.formatDate(calendar.getTime(), 1) + ")";
		}
...
%>
    <tr>
      ...
      <%}else{%>  
			...
			<%}
			%>      </td>
      <td nowrap class="thinborder">&nbsp;<%=strAmTimeIn%><%=(strAmTimeIn == null || strAmTimeIn.equals("&nbsp;") || strAmTimeIn.length() == 0 )? "" : strAmTimeIn2 %></td>
      <td nowrap class="thinborder">&nbsp;<%=strAmTimeOut%><%=(strAmTimeOut == null || strAmTimeOut.equals("&nbsp;") || strAmTimeOut.length() == 0 )? "" : strAmTimeOut2 %></td>
      <td nowrap class="thinborder">&nbsp;<%=WI.getStrValue(strPmTimeIn,"&nbsp;")%><%=(strPmTimeIn == null || strPmTimeIn.equals("&nbsp;") || strPmTimeIn.length() == 0 )? "" : strPmTimeIn2 %></td>
      <td nowrap class="thinborder">&nbsp;<%=WI.getStrValue(strPmTimeOut,"&nbsp;")%><%=(strPmTimeOut == null || strPmTimeOut.equals("&nbsp;") || strPmTimeOut.length() == 0 )? "" : strPmTimeOut2 %></td>
	<%
			iHours = 0;
			iMinutes = 0;
			 if (vHoursWork != null && vHoursWork.size()  > 0 &&
	...
	<tr>  	
    <td  align="right" class="thinborder" colspan="5">&nbsp;TOTAL&nbsp; </td>
    <td align="center" class="thinborder"  colspan="2">&nbsp;<%=CommonUtil.formatFloat(dEmpTotalHours, true)%> hours</td>
    </tr>	
  </table>
...
-->
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
      <td height="4" align="center" valign="bottom" bgcolor="#FFFFFF" class="thinborderBOTTOM"><font size="2"><strong>
	  
		<%=WI.formatName((String)vEmpDetails.elementAt(1),
	  (String)vEmpDetails.elementAt(2), (String)vEmpDetails.elementAt(3), 7)%>	
    ...
    <td width="6%">&nbsp;</td>
    <td width="47%" onClick="copyLabel();"><label id="copy_label"></label></td>
  </tr>
</table>
<% 

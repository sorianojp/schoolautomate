...
String strHeaderRemark = WI.fillTextValue("remarks");
String strFooterRemark = null;
if(strHeaderRemark.length() > 0) {
	int iIndexOf = strHeaderRemark.indexOf("##");
	if(iIndexOf > -1) {
		strFooterRemark = strHeaderRemark.substring(iIndexOf + 3);
		strHeaderRemark = strHeaderRemark.substring(0,iIndexOf);
	}		
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	...
	<tr><td height="70px;" align="center" valign="top" style="font-weight:bold"><font style="font-size:14px">OFFICE OF THE REGISTRAR</font>
	<br><br><br>
	<font style="font-weight:bold; font-size:20px"><u>C</u> <u>E</u> <u>R</u> <u>T</u> <u>I</u> <u>F</u> <u>I</u> <u>C</u> <u>A</u> <u>T</u> <u>I</u> <u>O</u> <u>N</u></font><br></td>
	</tr>
	<tr><td height="40px;" valign="bottom">TO WHOM IT MAY CONCERN:</td></tr>
	<tr>
		<td style="padding-left:50px; padding-right:40px; text-indent:50px;" height="40px" valign="bottom">This is to certify that <strong><%=WI.formatName((String)vResSummary.elementAt(1),(String)vResSummary.elementAt(2),(String)vResSummary.elementAt(3),7)%></strong>  <%=strHeaderRemark%>: </td>
	</tr>
...
<%}
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" id="myTable1" onClick="findRowNumber(event.target||event. srcElement);">
    ...
    <tr>
      <td>&nbsp;</td>
      <td width="15%"  height="19"><div align="left" style="padding-left:20px;"><strong>Course Code</strong></div></td>
      <td width="32%"><div align="left"><strong>Course Description</strong></div></td>
<%if(!bolIsScholastic && false){%>
...
<%}%>
      <td width="8%" align="center"><strong><%if(bolIsCGH && false){%>FINAL<%}%> Grade</strong></td>
<%if(bolIsCGH && false){%>
      <td width="8%" style="font-size:9px; font-weight:bold" align="center">COMPLETION GRADE</td>
<%}%>
      <td width="8%" style="font-weight:bold" align="center">Units</td>
<%if(!bolIsScholastic && false){%>
...
<%}%>    </tr>
    ...
    <tr id="row_<%=iRowID++%>">
      <td width="1%">&nbsp;</td>
      <td colspan="2"><span style="text-decoration:underline"><%=astrConvertSem[iSem]%> 
        <%=(String)vResDetail.elementAt(i+10) + " - "+(String)vResDetail.elementAt(i+11)%></span></td>
<%if(!bolIsScholastic && false){%>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
<%}if(!bolIsCGH && false){%>
...
<%}%>
    ...
    <tr>
      <td width="1%">&nbsp;</td>
      <td colspan="2"><%=astrConvertSem[iSem]%> 
        <%=(String)vResDetail.elementAt(i+10) + " - "+(String)vResDetail.elementAt(i+11)%></td>
<%if(!bolIsScholastic && false){%>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
<%}if(!bolIsCGH && false){%>
...
//System.out.println(vResDetail.elementAt(j+4));System.out.println(vResDetail.elementAt(j+5));
    ...
    <tr id="row_<%=iRowID++%>">
      <td>&nbsp;</td>
      <td  height="19"><div align="left" style="padding-left:20px;"><%=(String)vResDetail.elementAt(j+2)%></div></td>
...
<%}if(!bolIsCGH && false){%>
      <td align="center">
<%if(!bolIsCGH){%><%=WI.getStrValue(vResDetail.elementAt(j+8))%><%}else{
...
<%}%>
    ...
    <%if(vResDetail.elementAt(j+12) != null && false){%>
    <tr id="row_<%=iRowID++%>">
      <td>&nbsp;</td>
...
<%
if(strFooterRemark != null && strFooterRemark.length() > 0){
%>
  <tr>
      <td colspan="2" valign="top" align="center"><%=WI.getStrValue(strFooterRemark)%></td>
    </tr>
<%}%>
  <tr>
    <td width="59%" height="60" valign="top"></td>
	
	
	<td valign="bottom" width="41%" align="right">
	<div align="center">
		<strong><u><%=WI.getStrValue(WI.fillTextValue("pos_new"), WI.getStrValue(CommonUtil.getNameForAMemberType(dbOP,"University Registrar",7)))%></u></strong><br>
        <%=WI.getStrValue(WI.fillTextValue("des_new"), "University Registrar")%>	</div>	</td>
  </tr>
</table>
</body>
</html>

...
if(strSchCode == null)
	strSchCode = "";
if(strSchCode.startsWith("CGH") || strSchCode.startsWith("PHILCST") || strSchCode.startsWith("UPH")) {
	strFontSize = "10px";
	bolIsCGH = true;
}
%>
...
//depends on course/major selected.. 
String strCourseToDisp = null;
String strMajorToDisp  = null;
...
if(WI.fillTextValue("stud_id").length() > 0)
{
	...
	if(vResSummary == null)
		...
		strCourseToDisp = (String)vResSummary.elementAt(6);
		strMajorToDisp  = (String)vResSummary.elementAt(7);
		...
		if (WI.fillTextValue("selCourse").length()>0) {
			...
			if(strTempQuery != null) {
				strSQLQuery = "select COURSE_NAME, major.MAJOR_NAME from stud_curriculum_hist "+
					"join SEMESTER_SEQUENCE on (SEMESTER_SEQUENCE.semester_val = stud_curriculum_hist.semester) "+
					...
					"where user_index = "+	(String)vResSummary.elementAt(0)+strTempQuery+" and STUD_CURRICULUM_HIST.is_valid = 1 order by sy_from desc,SEM_ORDER desc";
				rs = dbOP.executeQuery(strSQLQuery);
				if(rs.next()) {
					strCourseToDisp = rs.getString(1);
					strMajorToDisp  = rs.getString(2);
				}
				rs.close();
...
//sSystem.out.println(vResDetail);
...
if((strSchCode.startsWith("UDMC") || bolIsCGH) && vResDetail != null && vResDetail.size() > 0) {
	...
	"where user_index = "+	(String)vResSummary.elementAt(0)+" and is_valid = 1 ";
	if(strTempQuery != null)
		strSQLQuery += strTempQuery;
	strSQLQuery += " order by sy_from desc,SEM_ORDER desc";
...
<%if(bolIsCGH){%>
    ...
    <tr>
	  ...
	  <strong><font size="2">Summary of Student Grade</font></strong></div></td>
    </tr>
  </table>
<%}if(vResSummary != null && vResSummary.size()>0){%>
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
    <tr bgcolor="#FFFFFF">
      <td height="19" width="2%" class="thinborderTOP">&nbsp;</td>
      <td width="10%" class="thinborderTOP">Student: <strong></strong></td>
      <td width="58%" class="thinborderTOP"><strong><%=WI.formatName((String)vResSummary.elementAt(1),(String)vResSummary.elementAt(2),(String)vResSummary.elementAt(3),1)%></strong></td>
      <td width="7%" class="thinborderTOP">Year: </td>
      <td width="23%" class="thinborderTOP"><strong><%=astrConvertYear[Integer.parseInt(WI.getStrValue(vResSummary.elementAt(10),"0"))]%></strong></td>
    </tr>
    <tr bgcolor="#FFFFFF">
      <td width="2%" height="19">&nbsp;</td>
      <td>Course:</td>
      <td><strong><%=strCourseToDisp%>
	  <%=WI.getStrValue(strMajorToDisp," Major in ","","")%> (<%=(String)vResSummary.elementAt(8)%>-<%=(String)vResSummary.elementAt(9)%>)</strong></td>
      <td>Date:</td>
...
<%if(WI.fillTextValue("hide_1").length() == 0){%>
    <tr bgcolor="#FFFFFF">
      <td height="25">&nbsp;</td>
...
<%}%>
	  ...
	  <%if(bolIsCGH){
			...
			<%=strTemp%>
		<%}else{%>
        	<%=(String)vResDetail.elementAt(j+6)%>
		<%}%>
...
{%>
  ...
  <table width="100%" border="0" bgcolor="#FFFFFF" cellpadding="0" cellspacing="0">
    ...
    <tr>
      <td align="center">&nbsp;</td>
      <td colspan="7"><font size="1"><strong><u><%=iSem%> SEMESTER,
...
<table cellpadding="0" cellspacing="0" width="100%" border="0">
  ...
  <%}%>
  <tr>
    <td width="59%" height="121" valign="bottom"><div align="left"><br>
...
<%}%>
	...
	<td width="41%" align="right"><div align="center"><strong><font size="2"><br><br><br>
...
%>	<%=WI.getStrValue(WI.fillTextValue("pos_new"), strTemp)%><br>
        ...
        <%if(!strSchCode.startsWith("CLDH") && !strSchCode.startsWith("CGH") && !strSchCode.startsWith("UDMC") && !strSchCode.startsWith("DBTC") && !strSchCode.startsWith("FATIMA")){%>

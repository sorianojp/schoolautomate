...
<body>
<%@ page language="java" import="utility.*,enrollment.GradeSystem,student.StudentEvaluation,enrollment.GradeSystemExtn2,java.util.Vector" %>
<%
...
//add security here.
	try
	{
		dbOP = new DBOperation((String)request.getSession(false).getAttribute("userId"),
								"Admin/staff-Registrar Management-RESIDENCY STATUS MONITORING","residency_status_print_UPHSD_detailed.jsp");
	}
	catch(Exception exp)
	{
		exp.printStackTrace();
...
GradeSystemExtn2 GSExtn = new GradeSystemExtn2();
Vector vGradeNameList = new Vector();
Vector vGradeList = new Vector();
Vector vGradeAverage = new Vector();
int iColSpan = 0;
int iIndex = 0;
String strSubCode = null;
String strGrade = null;
String strSYFromSemester = null;
...
if(WI.fillTextValue("stud_id").length() > 0) {
	...
	if (WI.fillTextValue("selCourse").length()>0)
		...
		vGradeList = GSExtn.getStudentResidencyGrade(dbOP, (String)vTemp.elementAt(0));
		if(vGradeList != null && vGradeList.size() > 0){
			vGradeNameList = (Vector)vGradeList.remove(0);
			vGradeAverage = (Vector)vGradeList.remove(0);
		}

		iColSpan = vGradeNameList.size();		
	}
...
<br><%=SchoolInformation.getAddressLine1(dbOP,false,false)%><br><u>Detailed Student Grade</u>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
	<tr>					
		...
		<%
		for(int x = 0; x < vGradeNameList.size(); x++){
			strTemp = (String)vGradeNameList.elementAt(x);
			if(strTemp == null)
				continue;				
		...
		%>
		<td width="5%" class="thinborderTOPBOTTOM" align="center"><%=strTemp%></td>
		<%}%>
		<td width="5%" class="thinborderTOPBOTTOM" align="center">AVE</td>
		<td width="5%" class="thinborderTOPBOTTOM" align="center">GRADE</td>
		<td width="5%" class="thinborderTOPBOTTOM" align="center">COMP</td>
	</tr>
	...
	<tr>      	
		...
		strSYFromSemester = (String)vResDetail.elementAt(i+10)+"-"+Integer.toString(iCurSem);
		%>
    	<td colspan="<%=iColSpan%>" class="thinborderTOPBOTTOM">
    ...
    <tr>     
	<%
		strSubCode = (String)vResDetail.elementAt(i+2);
	%> 	
      	<td height="19"><%=strSubCode%></td>
	...
	for(int x = 0; x < vGradeNameList.size(); x++){
		...
		strGrade = "";			
		for(int k = 0; k < vGradeList.size(); k+=4){			
			if(!((String)vGradeList.elementAt(k + 2)).equals(strSYFromSemester))
				continue;
			if(!((String)vGradeList.elementAt(k)).equals(strSubCode))
				continue;				
			if((String)vGradeList.elementAt(k + 1) == null)		
				continue;			
			if(!((String)vGradeList.elementAt(k + 1)).equals(strTemp))
				continue;				
			strGrade = WI.getStrValue((String)vGradeList.elementAt(k + 3));
			strGrade = CommonUtil.formatFloat(strGrade,true);
			break;			
		}	
	%>
	...
	<%}
		...
		strGrade = "";
		iIndex = vGradeAverage.indexOf(strSubCode+"-"+strSYFromSemester);
		if(iIndex > 0){
			strGrade = WI.getStrValue((String)vGradeAverage.elementAt(iIndex + 1));
			strGrade = CommonUtil.formatFloat(strGrade,true);
		}
	%>
		<td align="center"><%=strGrade%></td>
	...
	<tr>							
		...
		iColSpan = iColSpan + 6;
		%>
		<td colspan="<%=iColSpan%>" align="center" class="thinborderTOP">&nbsp;</td>
	</tr>

...
String strSQLQuery = "select sg_index from SUB_GROUP_MAP_FORM19 where course_index_ = "+vStudInfo.elementAt(5);
strSQLQuery = dbOP.getResultOfAQuery(strSQLQuery, 0);
if(strSQLQuery != null) {
	strSQLQuery = "update SUBJECT_GROUP set GROUP_NAME = (select SG_NAME from SUB_GROUP_MAP_FORM19 where sg_index = GROUP_INDEX and course_index = "+
...
strSQLQuery = "select SG_NAME from SUB_GROUP_MAP_FORM19 order by ORDER_NO";
rs = dbOP.executeQuery(strSQLQuery);
Vector vTemp = new Vector();
vTemp.addElement(vSubGroupInfo.remove(0));
int iIndexOf = 0;
while(rs.next()) {
...
%>
  ...
  <tr>
    ...
    <!--<td class="thinborderLEFT">&nbsp; <%if(iIndexOfSubGroup == 11){%> <%=strTemp%> <%}%></td>
    <td class="thinborderLEFT">&nbsp; <%if(iIndexOfSubGroup == 12){%> <%=strTemp%> <%}%></td>-->
	<%for(int j = 0; j < iAddlGroup; ++j) {%>
...
int iIndex = 0;%>
  <tr>
    ...
    <!--<td align="center" class="thinborderTOPLEFTBOTTOM">&nbsp;<%=CommonUtil.formatFloat(dSG11,false)%></td>
    <td align="center" class="thinborderTOPLEFTBOTTOM">&nbsp;<%=CommonUtil.formatFloat(dSG12,false)%></td>-->
	<%for(int j = 0; j < iAddlGroup; ++j) {
		if(j == 0)

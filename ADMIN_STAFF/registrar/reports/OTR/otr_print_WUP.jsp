...
String strSQLQuery = "select sy_from, semester, sub_code, sub_name, IS_REMOVAL_WUP,DATE_SUBMITTED_WUP, g_sheet_final.sub_sec_index,is_verified,nstp_val from g_sheet_final "+
				...
				"left join faculty_load on (faculty_load.sub_sec_index = g_sheet_final.sub_sec_index) "+
				" 		and faculty_load.is_valid = 1 "+
				"where user_index_ = "+
				(String)vStudInfo.elementAt(12)+" and g_sheet_final.is_valid = 1";
java.sql.ResultSet rs = dbOP.executeQuery(strSQLQuery);				
while(rs.next()) {
	if(rs.getInt(5) == 1) {
		vRemovalGradeInfo.addElement(rs.getString(1));//[0] sy_from
		vRemovalGradeInfo.addElement(rs.getString(2));//[1] semester
		if(rs.getString(9) != null) 
			vRemovalGradeInfo.addElement(rs.getString(3)+"("+rs.getString(9)+")");//[2] sub_code
		else
			vRemovalGradeInfo.addElement(rs.getString(3));//[2] sub_code
		vRemovalGradeInfo.addElement(rs.getString(4));//[3] sub_name
		
		continue;
	}
	if(rs.getString(7) != null) {
		if(rs.getString(6) == null) {//not submitted
			vNotSubmittedGrade.addElement(rs.getString(1));
			vNotSubmittedGrade.addElement(rs.getString(2));
			if(rs.getString(9) != null) 
				vNotSubmittedGrade.addElement(rs.getString(3)+"("+rs.getString(9)+")");//[2] sub_code
			else
				vNotSubmittedGrade.addElement(rs.getString(3));
			vNotSubmittedGrade.addElement(rs.getString(4));
		}
		else if(rs.getInt(8) == 0) {
			vNotVerifiedGrade.addElement(rs.getString(1));
			vNotVerifiedGrade.addElement(rs.getString(2));
			if(rs.getString(9) != null) 
				vNotVerifiedGrade.addElement(rs.getString(3)+"("+rs.getString(9)+")");//[2] sub_code
			else
				vNotVerifiedGrade.addElement(rs.getString(3));
			vNotVerifiedGrade.addElement(rs.getString(4));
		}
	}
}
rs.close();
...
if(vRetResult.elementAt(i+ 3) != null) {//null only if accredited subject - foreign stud.	
	...
	if(false && vRetResult.elementAt(i) != null && bolIsSchNameNull) {//cross enrolled.
	...
	//cross enrolled.. 
	if(false && vRetResult.elementAt(i) != null && bolIsSchNameNull) {//display will be schooo name<br>sy/term (cross enrolled)
		strSYToDisp = null;
		strSemToDisp = null;		
...
%>
  ...
  <tr bgcolor="#FFFFFF">  	    	
  	<td colspan="4"><label id="<%=++iLevelID%>" onClick="UpdateLabel('<%=iLevelID%>')"><u><i><%=strTemp%></i></u></label>	</td>
  </tr>
...
//it is re-exam if student has INC and cleared in same semester,
	...
	if(vRetResult.size() > (i + 16) && vRetResult.elementAt(i + 8) != null && vRetResult.elementAt(i + 6) != null  && vRetResult.elementAt(i + 6 + 11) != null && 
      ...
      strTemp = (String)vRetResult.elementAt(i + 8 + 11);	  	  
	  	...
	  	/*** 
	  		piece of code for removal grade, if not submitted grade, and if not verified grade.. 
		**/	
			strRemovalGrade = "&nbsp;";
			for(int a = 0; a < vRemovalGradeInfo.size(); a += 4) {
				if(vRemovalGradeInfo.elementAt(a).equals(WI.getStrValue(vRetResult.elementAt(i + 1))) && //sy_from
					vRemovalGradeInfo.elementAt(a + 1).equals(WI.getStrValue(vRetResult.elementAt(i + 3))) && //semester.
					vRemovalGradeInfo.elementAt(a + 2).equals(WI.getStrValue(vRetResult.elementAt(i + 6))) && //sub_code
					vRemovalGradeInfo.elementAt(a + 3).equals(WI.getStrValue(vRetResult.elementAt(i + 7))) ){//sub_name
						strRemovalGrade = strGrade;
						strGrade        = "&nbsp;";
						
						vRemovalGradeInfo.remove(a);vRemovalGradeInfo.remove(a);vRemovalGradeInfo.remove(a);vRemovalGradeInfo.remove(a);
						
						break;
				}
			}
			for(int a = 0; a < vNotSubmittedGrade.size(); a += 4) {
				//if grade not submitted.
				if(vNotSubmittedGrade.elementAt(a).equals(WI.getStrValue(vRetResult.elementAt(i + 1))) && //sy_from
					vNotSubmittedGrade.elementAt(a + 1).equals(WI.getStrValue(vRetResult.elementAt(i + 3))) && //semester.
					vNotSubmittedGrade.elementAt(a + 2).equals(WI.getStrValue(vRetResult.elementAt(i + 6))) && //sub_code
					vNotSubmittedGrade.elementAt(a + 3).equals(WI.getStrValue(vRetResult.elementAt(i + 7))) ) {//sub_name
						strGrade = "NGS";
						strCE = "&nbsp;";//units.
						
						vNotSubmittedGrade.remove(a);vNotSubmittedGrade.remove(a);vNotSubmittedGrade.remove(a);vNotSubmittedGrade.remove(a);
						break;
	
				}//not yet deans approval.
			}
			
			if(!strGrade.equals("NGS")){
				for(int a = 0; a < vNotVerifiedGrade.size(); a += 4) {
					if(vNotVerifiedGrade.elementAt(a).equals(WI.getStrValue(vRetResult.elementAt(i + 1))) && //sy_from
						vNotVerifiedGrade.elementAt(a + 1).equals(WI.getStrValue(vRetResult.elementAt(i + 3))) && //semester.
						vNotVerifiedGrade.elementAt(a + 2).equals(WI.getStrValue(vRetResult.elementAt(i + 6))) && //sub_code
						vNotVerifiedGrade.elementAt(a + 3).equals(WI.getStrValue(vRetResult.elementAt(i + 7))) ) {//sub_name
							strGrade = "WDA";
							strCE = "&nbsp;";//units.
							
							vNotVerifiedGrade.remove(a);vNotVerifiedGrade.remove(a);vNotVerifiedGrade.remove(a);vNotVerifiedGrade.remove(a);
							break;
					}
	  			}
			}	  	
...
%>
  ...
  <td height="14">&nbsp; &nbsp; &nbsp; <%=WI.getStrValue(vRetResult.elementAt(i + 6),"&nbsp;")%></td>  
	...
	<td align="center"><label id="<%=++iLevelID%>" onClick="UpdateLabel('<%=iLevelID%>')"><%=WI.getStrValue(strRemovalGrade)%></label></td>
...
<% if (iLastPage == 1){%>
	<%if(WI.fillTextValue("addl_remark").length() > 0){%>
		<tr bgcolor="#FFFFFF">
			<td height="14" colspan="5"><%=WI.fillTextValue("addl_remark")%></td>
		</tr>
	<%}%>

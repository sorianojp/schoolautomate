...
if(strPrintPage.equals("1")) {
	String strSQLQuery = "select print_index from track_printing where date_printed = '"+WI.getTodaysDate()+"' and stud_index = "+
...
<tr>
	...
	<td width="50%"><strong><%=((String)vStudInfo.elementAt(2)).toUpperCase()%>,&nbsp;<%=WI.getStrValue((String)vStudInfo.elementAt(0)).toUpperCase()%> <%=WI.getStrValue(vStudInfo.elementAt(1)," ").toUpperCase()%></strong></td>
...
<table width="100%" border="0">
	<tr valign="top">
		<td width="135px" height="710px">&nbsp;</td>	  
	  <td>
...
if(vRetResult.elementAt(i+ 3) != null) {//null only if accredited subject - foreign stud.
	...
	if(((String)vRetResult.elementAt(i+ 3)) != null && ((String)vRetResult.elementAt(i+ 3)).length() == 1) {
						...
						String strSQLQuery = strGetTuitionType + (String)vRetResult.elementAt(i+ 1)+" and semester = "+(String)vRetResult.elementAt(i+ 3);
						strSQLQuery = dbOP.getResultOfAQuery(strSQLQuery,0);
...
if(strCE != null && strCE.length() > 0 && strCE.indexOf(".") > -1 && (strCE.endsWith(".0") || strCE.endsWith(".00")))
	strCE = strCE.substring(0, strCE.indexOf("."));

%>
	
  <tr bgcolor="#FFFFFF">      
	<td height="14">&nbsp;<label id="<%=++iLevelID%>" onClick="UpdateLabel('<%=iLevelID%>')"><%=WI.getStrValue(strTemp).toUpperCase()%></label></td>  
    <td >&nbsp;&nbsp;&nbsp;&nbsp;<label id="<%=++iLevelID%>" onClick="UpdateLabel('<%=iLevelID%>')"><%=WI.getStrValue(WI.getStrValue(vRetResult.elementAt(i + 7)).toUpperCase(),"&nbsp;")%></label></td>
...
<tr>
	<td colspan="5">
		<table width="100%" border="0">
			...
			<tr>
				<td width="2%">&nbsp;</td>
				<td align="center" class="thinborderTOP"><font size="2">OIC, REGISTRAR</font></td>
				<td width="6%">&nbsp;</td>

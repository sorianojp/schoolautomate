...
<body topmargin="0" bottommargin="0" onLoad="<%if(WI.fillTextValue("donot_print").length() == 0) {%>window.print();<%}%>">
...
if(strSubSecIndex != null) {//get here subject section detail. 
	...
	strOfferedByCollege = dbOP.mapOneToOther("e_sub_section","sub_sec_index", strSubSecIndex, 
											"OFFERED_BY_COLLEGE", "");
	//if general education subject, college is cas.. 
	if(WI.getStrValue(dbOP.getResultOfAQuery("select group_index from subject where sub_index = "+WI.fillTextValue("subject"),0)).equals("84"))
		strOfferedByCollege = "23";
...
if(strSubSecIndex != null) {
	strDateSubmitted = "select DATE_SUBMITTED_WUP from faculty_load where sub_sec_index = "+strSubSecIndex+
						" and is_valid = 1 and DATE_SUBMITTED_WUP is not null";
	rs = dbOP.executeQuery(strDateSubmitted);
	if(rs.next())
		strDateSubmitted = ConversionTable.convertMMDDYYYY(rs.getDate(1));
	else	
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <% if (!strSchCode.startsWith("AUF")) {%>
    <tr> 
      <td width="50%">&nbsp;&nbsp;SY/TERM : <strong><%=astrConvSemester[Integer.parseInt(WI.fillTextValue("offering_sem"))]%>, <%=WI.fillTextValue("sy_from")%> - <%=WI.fillTextValue("sy_to")%> </strong></td>
      <td width="28%">&nbsp;</td>
      <td width="22%" align="center"><%=WI.getStrValue(strDateSubmitted, "&nbsp;")%></td>
    </tr>
    <tr> 
      <td>&nbsp;</td>
      <td>&nbsp;</td>
      <td class="thinborderTOP"><div align="center"><font size="1">DATE SUBMITTED</font></div></td>
    </tr>
    ...
    <tr> 
	  ...
	  Vector vPMTSchedule = (Vector)vRetResult.elementAt(0);
		...
		for (i = 0; i < iNumGrading; ++i){
			...
			if(bolIsSummer && strTemp.startsWith("M"))
				continue;
			...
			if (iIndexOf != -1) 
				...
				if (strTemp.toLowerCase().startsWith("final")) 
					strTemp = "Final Term Grade";
				else if(!strTemp.endsWith("Grade"))
					strTemp = strTemp + " Grade";
      ...
      <td width="8%"  class="thinborder"><div align="center"><font color="#000099" size="1"><strong>Equivalent</strong></font></div></td>
    ...
    <tr> 
      ...
      <% for (k = 0; k < iNumGrading; ++k) {
		...
		try {
			...
			if(strGrade.length() == 2)
				strGrade = strGrade + ".00";
			else if(strGrade.endsWith(".0"))
				strGrade = strGrade + "0";
		}
		catch(Exception e) {}

		strAvgGrade = WI.getStrValue(strAvgGrade, "&nbsp;");
		if(strAvgGrade.endsWith(".0"))
			strAvgGrade = strAvgGrade + "0";
	  %>
      <td align="center"  class="thinborderGrade"<%=strFontColor%>><font size="1"<%=strFontColor%>><strong><%=strGrade%></strong></font></td>
      <td align="center"  class="thinborderGrade"<%=strFontColor%>><font style="font-size:7px;"<%=strFontColor%>><strong><%=WI.getStrValue(strAdmSlipNumber)%></strong></font></td>
	  	
      <%} //end for loop k = 0; k < iNumGrading; ++k%>
      <td align="center"  class="thinborder"><font size="1"<%=strFontColor%>><strong><%=strAvgGrade%></strong></font></td>
	...
	<tr>
		...
		<td width="48%" valign="top">
			<table width="94%" border="0" cellpadding="0" cellspacing="0">
				  <tr>
					<td colspan="2" valign="bottom">APPROVED BY:</td>
				  </tr>
					<tr> 
					  <td height="17" colspan="2" valign="bottom">&nbsp;</td>
					</tr>
				  <tr> 
					<td width="15%" height="23" valign="bottom">&nbsp;</td>
					<td width="85%" valign="bottom">
					<div align="center"> 
					  <table width="88%" border="0" cellspacing="0" cellpadding="0">
						<tr> 
						  <td class="thinborderBottom"><div align="center"><%=WI.getStrValue(dbOP.mapOneToOther("COLLEGE","C_INDEX",strOfferedByCollege,"DEAN_NAME"," and is_del = 0")).toUpperCase()%></div></td>
						</tr>
					  </table>
					</div>

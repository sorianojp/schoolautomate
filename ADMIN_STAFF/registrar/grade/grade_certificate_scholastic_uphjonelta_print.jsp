...
boolean bolHidePercentGrade = false;
if(WI.fillTextValue("hide_percent_grade").length() > 0) 
	bolHidePercentGrade = true;
%>
...
<tr>
    <td height="25" valign="bottom" style="line-height:20px;font-size:12px;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;This is to certify that based on records on file,      <strong><%=((String)vStudInfo.elementAt(1)).toUpperCase()%></strong> had taken
	  <strong>
	  <%
	   if (((String)vStudInfo.elementAt(2)).equals("LAW")) {
	     out.print("BACHELOR OF LAWS");
		}
		else 
		if (((String)vStudInfo.elementAt(2)).equals("MARINE TRANSPORTATION")) {
	     out.print("BACHELOR OF SCIENCE IN MARINE TRANSPORTATION");
		}
		else 
		{strTemp = "select course_name from course_offered where is_valid = 1 and course_code ="+WI.getInsertValueForDB(strCourseToDisp,true, null);
		strTemp = dbOP.getResultOfAQuery(strTemp, 0);
		if(strTemp != null)
			strCourseToDisp=strTemp;
		out.print(strCourseToDisp);
		}
	  ...
	  <%=WI.getStrValue(strMajorToDisp," Major in ","","")%></strong>
...
<%}%>
	...
	if (vGradeDetail !=null) {
		...
		if(strGradeValue != null && (strGradeValue.endsWith(".00") || strGradeValue.length() == 3) && strGradeValue.indexOf(".") > -1)

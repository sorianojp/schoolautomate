...
<body topmargin="0" bottommargin="0" onLoad="duplicateLabel();CallOnLoad();window.print()" bgcolor="#DDDDDD">
...
int iNoOfStud = vStudList.size() - 1;
int iShowCon = Integer.parseInt(WI.getStrValue(WI.fillTextValue("show_con"), "3"));//3 = show all, 1 = show registrar, 2 = show student's copy.
...
<label for="show_registrar<%=vStudList.size()%>"></label>
  <table width="100%" border="0" cellspacing="0" cellpadding="0" id="show_registrar<%=vStudList.size()%>">
    <tr >
      <td valign="top">&nbsp;</td>
...
<label for="registrar_copy<%=vStudList.size()%>">&nbsp;</label>
<table cellpadding="0" cellspacing="0" border="0" width="100%" id="registrar_copy<%=vStudList.size()%>"><tr><td>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
...
<%if(iShowCon == 3 || iShowCon == 1){%>
	<DIV style="page-break-before:always" >&nbsp;</DIV>
<%}%>
<label for="show_student<%=vStudList.size()%>"></label>
  <table width="100%" border="0" cellspacing="0" cellpadding="0" id="show_student<%=vStudList.size()%>">
    <tr >
      <td valign="top">&nbsp;</td>
      <td align="right" valign="top" style="font-size:9px;">PHILCSTREG#17 </td>
    </tr>
...
<label id="student_copy<%=vStudList.size()%>"></label>

<%if(vStudList.size() > 0) {%>
	<%if(iShowCon == 3 || iShowCon == 2){%>
...
function duplicateLabel() {
	duplicateLabelFinal(<%=iNoOfStud%>);
	hideCopyToPrint(<%=iNoOfStud%>);
}
function duplicateLabelFinal(iNoOfStud) {
	<%if(iShowCon == 1){%>
		return;
	<%}%>
	for(var i = 0; i <= iNoOfStud; ++i) {
		if(!document.getElementById("student_copy"+i))
			continue;
		document.getElementById("student_copy"+i).innerHTML = document.getElementById("registrar_copy"+i).innerHTML;
	}
}
//hide registrar/student copy.
function hideCopyToPrint(iNoOfStud) {
	<%if(iShowCon == 3){%>
		return;
	<%}%>
	var iShowCon = <%=iShowCon%>;
	var obj;
	for(var i = 0; i <= iNoOfStud; ++i) {
		if(iShowCon == 1) {//show only registrar copy.
			document.getElementById("show_student"+i).innerHTML = "";
			//document.getElementById("student_copy"+i).innerHTML = ""; -- it is not set..
		}
		else {
			document.getElementById("show_registrar"+i).innerHTML = "";
			document.getElementById("registrar_copy"+i).innerHTML = "";
		}
	}
}
</script>

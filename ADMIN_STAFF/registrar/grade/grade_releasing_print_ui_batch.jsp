<%
	...
	if(strSchCode.startsWith("UL")){%>
		...
		<jsp:forward page="./grade_releasing_print_onepage_batch.jsp" />
	<%return;}
...
//System.out.print("      Remaining to print : "+vStudList.size());
	...
	if(vStudInfo == null)
		continue;
	Thread.sleep(200);
	bolShowGWA = WI.fillTextValue("show_gwa").equals("1");
	...
	if(vGradeDetail == null)
		...
		if(strTemp != null && strTemp.length() > 0){
			 ...
			 if (strGradeFor.toLowerCase().startsWith("final")) {
				  strGradeTableName = "G_SHEET_FINAL";
				  strGradeNameCon = " and COPIED_FR_TF_GS = 0 and user_index_=" + (String)vStudInfo.elementAt(0);
			}
			else 
			  strGradeNameCon = " and grade_name='" + ConversionTable.replaceString(strGradeFor, "'", "''") + "'";
	...
	if(strSchCode.startsWith("UDMC") || strSchCode.startsWith("CLDH") || strSchCode.startsWith("DBTC"))
...
<!-- End of printing information -->
	...
	if(!strSchCode.startsWith("CLDH")){
		if (!strSchCode.startsWith("UDMC") && !bolIsCGH){%>
				  ...
				  <%}else {if(bolIsCollegeOnly){%>COLLEGE OF<%}else{%>OFFICE OF THE UNIVERSITY<%}%>
				  REGISTRAR <%}%>
				  </strong></font>
				  <br>
				  <br>
	...
	<%	}
			...
			<strong>
			<%if(!strSchCode.startsWith("CLDH")){%><%=strGradeFor.toUpperCase()%><%}%></strong> GRADES FOR <strong><%=strTermName.toUpperCase()%></strong>
			<strong>, AY <%=strSYFrom%> - <%=strSYTo%></strong>
			<br>
			 </div></td>
		</tr>
		<%}%>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
	  ...
	  <tr>
		...
		<td width="11%" height="19" >Student ID </td>
		<td width="33%" ><strong><%=strStudID%></strong></td>
	...
	<td width="70%">&nbsp;<%=SchoolInformation.getSchoolName(dbOP,true,false)%> - Collegiate Grade Record</td>
		<td width="30%"><div align="right"><%=strTermName.toUpperCase()%> , <%=strSYFrom%> - <%=strSYTo%> </div></td>
	</tr>
	</table>
	<table border="0" cellpadding="0" cellspacing="0" width="100%">
	  <tr valign="bottom">
		...
		<td width="20%" class="thinborderBottom">&nbsp;<%=strStudID%></td>
	...
	<%}%>
		...
		<td width="16%" align="center" class="thinborder"><font size="1"><strong>REMARKS</strong></font></td>
		<% }%>
	  </tr>
	  <%
	   int iSubjCount = 0;
...
**/
	...
	if(strSchCode.startsWith("CGH")) {
		...
		int iIndexOf = vMathEnglEnrichment.indexOf(strSubCode);
	...
	//if(bolPutParanthesis) -- already taken care above.. 
	//	strGradeValue = "(3.0)";
			%>
			<%=strGradeValue%>
			<%}%>
	...
	%>
	  ...
	  <tr>
		...
		<td colspan="7" class="thinborder"><%=(String)vGradeDetail.elementAt(i+2)%></td>
	  </tr>
	  <%
...
<%}else if(!strSchCode.startsWith("DBTC") && !strSchCode.startsWith("UB")){
		...
		<%}%><br>otherwise they become failure
		<%}%>    </td>
	<%if(strSchCode.startsWith("AUF")){%>
		<td align="center">&nbsp;</td>
	<%}%>

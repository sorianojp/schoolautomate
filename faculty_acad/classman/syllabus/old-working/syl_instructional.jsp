...
function GoToParentWnd() {
	...
	window.opener.document.form_.inst_technique.value  = document.form_.inst_detail.value;
}
...
function AddToList() {
	...
	if(document.form_.inst_detail.value.length > 0) 
		document.form_.inst_detail.value = document.form_.inst_detail.value + "\r\n"+strAddToList;
	else	
		document.form_.inst_detail.value = strAddToList;
		
}
function SelMethodChanged() {
	if(document.form_.sel_method.selectedIndex == 0) 
		document.form_.inst_method.value = "";
	else	
		document.form_.inst_method.value = document.form_.sel_method[document.form_.sel_method.selectedIndex].text;
}
</script>
...
<%
	...
	if(WI.fillTextValue("page_action").length() > 0) {
		...
		try{
			strTemp = WI.fillTextValue("inst_method");
			if(strTemp.length() > 0) {
				strTemp = WI.getInsertValueForDB(strTemp, true, null);
				rs = dbOP.executeQuery("Select * from CM_SYL_PRELOAD_INSMETHOD where METHOD_NAME = "+
								strTemp);
				if(!rs.next()) {
					rs.close();
					///page action 2 = edit method name.
					if(WI.fillTextValue("sel_method").length() > 0 && WI.fillTextValue("page_action").equals("2")) //edit.
						dbOP.executeUpdateWithTrans("update CM_SYL_PRELOAD_INSMETHOD set METHOD_NAME="+
							strTemp+" where INST_METHOD_INDEX="+WI.fillTextValue("sel_method"), null, null, false);
					else			
						dbOP.executeUpdateWithTrans("insert into CM_SYL_PRELOAD_INSMETHOD (METHOD_NAME) values ("+
...
<form name="form_" method="post" action="./syl_instructional.jsp">
  <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
    <tr bgcolor="#6A99A2">
      <td height="25"><div align="center"><font color="#FFFFFF" size="2" face="Verdana, Arial, Helvetica, sans-serif"><strong>::::
          SYLLABUS INSTRUCTIONAL MAINTENANCE PAGE::::</strong></font></div></td>
    </tr>
    <tr>
  ...
  <table width="100%" border="0" cellpadding="5" cellspacing="0" bgcolor="#FFFFFF">
    <tr>
      <td width="19%">Instructional Method</td>
      <td width="81%">
          <select name="sel_method" onChange="SelMethodChanged();">
            <option value="">Select Method or Add New</option>
<%=dbOP.loadCombo("INST_METHOD_INDEX","method_name "," from CM_SYL_PRELOAD_INSMETHOD order by method_name",WI.fillTextValue("INST_METHOD_INDEX"), false)%>
          </select></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td><input type="text" name="inst_method" class="textbox" size="45" maxlength="128">
  ...
  <table width="100%" border="0" cellpadding="5" cellspacing="0" bgcolor="#FFFFFF">
    <tr bgcolor="#BDD5DF">
      <td height="25" colspan="2"><div align="center"><strong>LIST OF APPLICABLE
          INSTRUCTIONAL METHODS TO BE USED</strong></div></td>
    </tr>
    <tr>
...
if(strTemp == null) 	
	strTemp = WI.fillTextValue("inst_detail");
%>
	  <textarea name="inst_detail" cols="75" rows="10" onFocus="style.backgroundColor='#D3EBFF'" 
		onBlur="style.backgroundColor='white'" class="textbox" style="font-size:11px;"><%=WI.fillTextValue("inst_detail")%></textarea></td>
    </tr>
    <tr>
      <td height="25">&nbsp;</td>
      <td valign="top">
	  <a href="#"><img src="../../../images/clear.gif" border="0" onClick="document.form_.inst_detail.value='';"></a></td>
    </tr>
    <tr>
      <td height="25">&nbsp;</td>
      <td valign="top">&nbsp;</td>

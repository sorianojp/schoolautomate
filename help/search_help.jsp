...
<body bgcolor="#D2AE72" onLoad="document.form_.short_help.focus();" class="bgDynamic">
<%
	...
	try{
		dbOP = new DBOperation((String)request.getSession(false).getAttribute("userId"),
								"Admin/staff-HELP-Search Help","search_help.jsp");
	}
	catch(Exception exp){
		exp.printStackTrace();
		if(strErrMsg == null)
	...
	String[] astrSortByName    = {"Module","Sub-Module","Link"};
	String[] astrSortByVal     = {"module_name","sub_mod_name","link_name"};
	
	int iSearchResult = 0;
	
	SystemHelpFile shf = new SystemHelpFile();
	if(WI.fillTextValue("searchCollection").equals("1")){
		vRetResult = shf.searchHelp(dbOP, request);
		if(vRetResult == null)
...
%>
<form name="form_" method="post" action="search_help.jsp">
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
	<tr>
		<td height="25" colspan="3" bgcolor="#A49A6A">
			<div align="center"><font color="#FFFFFF"><strong>:::: SEARCH HELP PAGE ::::</strong></font></div></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<%
			...
			if(WI.fillTextValue("module").length() > 0)
				strSubModuleIndex= WI.getStrValue(strTemp, "0");
			else
				strSubModuleIndex= "0";
		%>
		<td>
		  <label id="load_sub_module">
			<select name="sub_module" onChange="loadLink();">
		...
		<label id="load_link">
			<select name="link">
				...
				<%=dbOP.loadCombo("link_index","link_name"," from application_links "+
					" where is_valid = 1 and sub_mod_index = "+strSubModuleIndex+
	...
	<tr>
		...
		<%if (WI.fillTextValue("type").equals("0")){%>
			<option value="0" selected>Link Help</option>
		<%}else{%>
			<option value="0">Link Help</option>
		
		<%}if (WI.fillTextValue("type").equals("1")){%>
			<option value="1" selected>Module Help</option>
		<%}else{%>
			<option value="1">Module Help</option>
		
		<%}if (WI.fillTextValue("type").equals("2")){%>
			<option value="2" selected>Sub-Module Help</option>
		<%}else{%>
			<option value="2">Sub-Module Help</option>
		<%}%>
		</select>		</td>
	</tr>
	<tr>
		...
		<td>Short Help: </td>
		<td>
			<select name="short_help_con" style="font-size:11px;">
				<%=shf.constructGenericDropList(WI.fillTextValue("short_help_con"),astrDropListEqual,astrDropListValEqual)%>
			</select>
			<input type="text" name="short_help" class="textbox" onFocus="style.backgroundColor='#D3EBFF'" 
				onBlur="style.backgroundColor='white'" size="32" maxlength="64" value="<%=WI.fillTextValue("short_help")%>">		</td>
	</tr>
	<tr>
		<td height="25">&nbsp;</td>
		<td>Long Help: </td>
		<td>
			<select name="long_help_con" style="font-size:11px;">
				<%=shf.constructGenericDropList(WI.fillTextValue("long_help_con"),astrDropListEqual,astrDropListValEqual)%> 
			</select>
			<input type="text" name="long_help" class="textbox" onFocus="style.backgroundColor='#D3EBFF'" 
				onBlur="style.backgroundColor='white'" size="32" maxlength="64" value="<%=WI.fillTextValue("long_help")%>">		</td>
	</tr>
	<tr>
		<td height="25">&nbsp;</td>
		<td>Sort by: </td>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="thinborder">
	...
	<tr bgcolor="#FFFFFF" align="center" style="font-weight:bold">
		...
		<td width="17%" class="thinborder">Module</td>
		<td width="17%" class="thinborder">Sub-Module</td>
		<td width="15%" class="thinborder">Link Name </td>
		<td width="20%" class="thinborder">Short Help </td>
		<td width="28%" class="thinborder">Long Help </td>
	</tr>
	<%
		int iCount = 1;
